...
class HomeScreen extends Component {
    ...
    checkLocation = async (address, addressLocation) => {
        var geoHash = null;
        var location = null;
            ...
            if (location.location === null) {
                if (location.error['PERMISSION_DENIED'] === 1) {
                    ...
                    setTimeout(() => {
                        Alert.alert(
                            translate("location"),
                            translate("location-permission-not-granted"),
                            [
                                {
                                    text: translate("choose-address"),
        ...
        if (hashData) {
            ...
            if (hashData?.cart?.validated === false && !this.state.geoHashChecked) {
                ...
                setTimeout(() => {
                    Alert.alert(
                        translate("you-want-to-change-address?"),
                        translate(hashData.cart.message),
                        [
                            { text: translate("cancel"), style: 'cancel' },
                            {
                                text: translate("confirm"), onPress: async () => {
                                    try {
                                        this.setState({ checking: true });
                                        await clearCart();
                                        this.props.setCartRestaurantNote('');
                                        this.props.setCartInfo([]);
                                        await this.setState({ activeAddress: address, currentLocation: location, geoHash })
                                        this.setState({ checking: false });
                                        this.onRefreshExplore(true)
                                    } catch (error) {
                                        this.setState({ checking: false });
                                        setTimeout(() => {
                                            Alert.alert('', error);
        ...
        await this.setState({ currentLocation: location, geoHash, geoHashChecked: true })
        return true;
    }

    async getExploreURL(collection, locationChecked) {
        if (!locationChecked) {
            const res = await this.checkLocation(this.state.activeAddress, this.state.currentLocation)
            if (!res) return null;
        }
    ...
    async onRefreshExplore(locationChecked) {
        ...
        const url = await this.getExploreURL(null, locationChecked)
        if (url) {
        ...
        if (this.props.signin.profile_info &&
            ...
            if (address) {
                ...
                Alert.alert(
                    ...
                    translate("active-address-deleted"),
                    [
                        {
    ...
    async onChangePosition(address, location) {
        this.setState({ checking: true })
        const res = await this.checkLocation(address, location);
        this.setState({ checking: false })
        if (res) {
                            ...
                            <TouchableOpacity
                                onPress={() => this.props.navigation.navigate(
                                    "searchfilter",
                                    {
                                        ...
                                        fulfillment: this.state.activeTabItem === 'delivery' ? 'delivery' : 'pickup'
                                    }
                                )}
                            >
                                ...
                                <View style={{ paddingVertical: 10 }} key={index}>
                                    ...
                                    this.state.restaurantsLoaded ? (
                                        <View style={{ justifyContent: 'center', alignItems: 'center', height: this.getRestContent(), paddingHorizontal: '10%' }}>
                                            <Text style={{ fontFamily: 'Circular Std', fontSize: 18, fontWeight: 'bold', color: '#1A1A1A', lineHeight: 24, textAlign: 'center' }}>{translate("no-restaurants-in-your-area")}</Text>
                                            <Text style={{ fontFamily: 'Circular Std', fontSize: 12, fontWeight: 'normal', color: '#1A1824', lineHeight: 18, textAlign: 'center', marginTop: 5 }}>{translate("no-res-desc1")}</Text>
                                            <TouchableOpacity
                                                onPress={
                                                    () => {
                                            ...
                                            >
                                                <Text style={{ fontFamily: 'Circular Std', fontSize: 12, fontWeight: 'normal', color: '#FF5D5D', textAlign: 'center', marginTop: 5 }}>{translate("suggest-restaurant")}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    ) : (null)
                                )
